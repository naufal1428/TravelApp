...
class OrderTicketFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        with(binding){
            ...
            editTextJenisTiket.setOnClickListener{
                val action = OrderTicketFragmentDirections.actionOrderTicketFragmentToTicketTypeFragment()
                findNavController().navigate(action)
            }

            findNavController().currentBackStackEntry?.savedStateHandle?.let {
                handle ->
                handle.getLiveData<String>("ticket")
                    .observe(viewLifecycleOwner){
                        res -> editTextJenisTiket.setText(res)
                    }
            }
